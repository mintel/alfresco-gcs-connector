...
public class GCSContentStoreIntegrationTest extends AbstractAlfrescoIT
{
    ...
    public void readWriteTest()
    {
        ...
        FileInfo node = this.getServiceRegistry().getFileFolderService().create(parentRef, "readWriteTest" + System.currentTimeMillis() + ".txt", ContentModel.TYPE_CONTENT);
    ...
    public void emojiTest()
    {
        String emoji = "☊☋☌☍☎☏☐☑☒☓☔☕☖☗☘☙☚☛☜☝☞☟☠☡☢☣☤☥☦☧☨☩☪☫☬☭☮☯☰☱☲☳☴☵☶☷☸☹☺☻☼☽☾☿♀♁♂♃♄♅♆♇♈♉♊♋♌♍♎♏♐♑♒♓♔♕♖♗♘♙♚♛♜♝♞♟♠♡♢♣♤♥♦♧♨♩♪♫♬♭♮♯≰≱≲≳≴≵≶≷≸≹≺≻≼≽≾≿⊀⊁⊂⊃⊄⊅⊆⊇⊈⊉⊊⊋⊌⊍⊎⊏⊐⊑⊒⊓⊔⊕⊖⊗⊘⊙⊚⊛⊜⊝⊞⊟⊠⊡⊢⊣⊤⊥⊦⊧⊨⊩⊪⊫⊬⊭⊮⊯⊰⊱⊲⊳⊴⊵⊶⊷⊸⊹";
        NodeRef parentRef = this.getCompanyHomeNodeRef();
        FileInfo node = this.getServiceRegistry().getFileFolderService().create(parentRef, "emojiTest" + System.currentTimeMillis() + ".txt", ContentModel.TYPE_CONTENT);
        NodeRef nodeRef = node.getNodeRef();
        assertNotNull(nodeRef);
    ...
    {
        NodeRef parentRef = this.getCompanyHomeNodeRef();
        FileInfo node = this.getServiceRegistry().getFileFolderService().create(parentRef, "node content test " + System.currentTimeMillis() + ".txt", ContentModel.TYPE_CONTENT);
